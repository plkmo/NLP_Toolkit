...
class puncLAS(nn.Module):
    def __init__(self, vocab_size, listener_embed_size, listener_hidden_size, output_class_dim, \
        ...
        self.speller2 = Speller(listener_hidden_size, listener_hidden_size, 2*listener_hidden_size, max_label_len2, \
                               output_class_dim2, num_layers=2)
    
    def forward(self, x, trg_input, trg_input2, infer=False):
        # x = batch_size X seq_len
        x = self.embed(x)
        x = self.listener(x)
        x1 = self.speller(x, trg_input, infer)
        x2 = self.speller2(x, trg_input2, infer)
        return x1, x2
